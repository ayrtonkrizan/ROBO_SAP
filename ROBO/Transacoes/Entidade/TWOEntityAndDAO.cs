    ...
    public class TWODAO
    {
        ...
        public static bool TransactionOpen()
        {
            try
            {
                ...
                {
                    rs.DoQuery("SELECT COUNT(*) FROM [Sage_Transaction_WO] WITH (NOLOCK) WHERE [TransactionType] = 40 AND ReadBySAP <> 'Y'");
                    if (rs.HasNext())
                    {
                    ...
                    {
                        principal.GravaAudit("Erro a consultar a tabela  Sage_Transaction08");
